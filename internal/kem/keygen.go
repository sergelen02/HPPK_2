...
type KEMPublic struct {
	...
	L      uint     `json:"L"`
	K      uint     `json:"K"`
	// Hidden coefficient matrices (hex strings row-major): Pp[i][j], Qp[i][j]
	Pp [][]string `json:"Pp"`
	Qp [][]string `json:"Qp"`
}
...
func LoadParams(pstr string, n, lambda, m int, L, K uint, nmin, nmax int64) *Params {
	p := new(big.Int)
	// 0: "0x..." 또는 10진 문자열 모두 허용
	p.SetString(pstr, 0)
	return &Params{P: p, N: n, Lambda: lambda, M: m, L: L, K: K, NoiseMin: nmin, NoiseMax: nmax}
}
